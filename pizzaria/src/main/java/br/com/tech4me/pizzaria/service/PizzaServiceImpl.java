...
public class PizzaServiceImpl implements PizzaService{
    ...
    public List<PizzaDTO> obterTodas() {
        return repositorio.findAll()
                        .stream()
                        .map(p -> new PizzaDTO(p.getId(), p.getSabor(), p.getIngredientes(), p.getQtdEmEstoque()))
                        .toList(); 
    }
    ...
    public Optional<PizzaCompletoDTO> obterPorId(String id) {
        ...
        if(pizza.isPresent()){
            return Optional.of(new PizzaCompletoDTO(pizza.get().getId(),
             ...
             pizza.get().getValor(), pizza.get().getQtdEmEstoque()));
        }
    ...
    public PizzaCompletoDTO cadastrar(PizzaCompletoDTO pizzaDto) {
        ...
        repositorio.save(p);
        return new PizzaCompletoDTO(p.getId(), p.getSabor(), p.getIngredientes(), p.getTamanho(), p.getValor(), p.getQtdEmEstoque());
    }
    ...
    public Optional<PizzaCompletoDTO> atualizarPorId(String id, PizzaCompletoDTO dto) {
        ...
        if (pizza.isPresent()) {
            ...
            pizzaAtualizar.getTamanho(), pizzaAtualizar.getValor(), pizzaAtualizar.getQtdEmEstoque()));
        }
